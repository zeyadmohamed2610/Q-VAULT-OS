...
#  Caches elevation for SUDO_CACHE_SECONDS (5 min) via SESSION.
#
#  Usage (from terminal):
#    from components.sudo_dialog import ask_sudo
#    granted = ask_sudo(parent_widget)
#    if granted:
...
class SudoDialog(QDialog):
    ...
    def _build_ui(self, command: str):
        ...
        body = QLabel(
            f"[sudo] Password for {user}:{cmd_text}\n"
        ...
        # Remaining cache info (if already granted)
        if SESSION.sudo_granted:
            rem = SESSION.sudo_remaining()
            note = QLabel(f"ℹ  Sudo already active ({rem}s remaining)")
            note.setStyleSheet(
    ...
    def _try_auth(self):
        ...
        granted = SESSION.sudo_request(pw)
        if granted:
            self._err.setText("")
            self.accept()
        else:
            ...
            if self._attempts >= 3:
                ...
                self._err.setText(
                    ...
                    f"{3 - self._attempts} attempt(s) remaining."
                )
            self._shake()
