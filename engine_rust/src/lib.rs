...
pub struct SecurityEngine {
    ...
    /// System fingerprint of the kernel modules at boot time.
    kernel_fingerprint: std::sync::Mutex<Option<Vec<u8>>>,
}
...
impl SecurityEngine {
    ...
    fn new(root_dir: &str) -> PyResult<Self> {
        ...
        Ok(Self {
            ...
            audit,
            sweeper_shutdown,
            kernel_fingerprint: std::sync::Mutex::new(None),
        })
    }
    ...
    /// Verify the integrity of a single audit log entry using the system audit key.
    fn verify_audit_log(&self, line: &str) -> bool {
        // We use the internal key already stored in the audit logger
        audit::verify_log_entry(line, self.audit.key())
    }

    /// Encrypt a binary file blob for an app's virtual storage.
    fn encrypt_file(&self, token: &str, plaintext: &[u8]) -> PyResult<Vec<u8>> {
        self.vault.encrypt_blob(token, plaintext)
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))
    }

    /// Decrypt a binary file blob for an app's virtual storage.
    fn decrypt_file(&self, token: &str, ciphertext: &[u8]) -> PyResult<Vec<u8>> {
        self.vault.decrypt_blob(token, ciphertext)
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))
    }

    /// Compute and verify the fingerprint of the kernel directory.
    /// If baseline is not set, this call establishes the 'Root of Trust'.
    /// Subsequent calls return (is_valid, current_fingerprint_hex).
    fn verify_kernel_integrity(&self) -> PyResult<(bool, String)> {
        use sha2::{Digest, Sha256};
        use std::fs;
        use std::io::Read;
        ...
        if !kernel_dir.exists() {
            return Err(pyo3::exceptions::PyRuntimeError::new_err("Kernel directory missing"));
        }

        let mut hasher = Sha256::new();
        
        // 1. Collect and sort all .py files for deterministic hashing
        let mut entries: Vec<PathBuf> = fs::read_dir(&kernel_dir)
        ...
        let mut baseline_lock = self.kernel_fingerprint.lock().unwrap();
        
        if baseline_lock.is_none() {
            // Initial boot: Establish baseline
            *baseline_lock = Some(current_hash);
            self.audit.log("INTEGRITY_BASE", None, &format!("Root of Trust established: {}", current_hex));
            Ok((true, current_hex))
        } else {
            // Runtime check: Compare against baseline
            let is_valid = baseline_lock.as_ref().unwrap() == &current_hash;
            if !is_valid {
                self.audit.log("INTEGRITY_VIOLATION", None, &format!("KERNEL TAMPERING DETECTED! Current: {}", current_hex));
            }
            Ok((is_valid, current_hex))
        }
    }
}
