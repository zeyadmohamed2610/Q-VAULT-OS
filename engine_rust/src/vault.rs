...
/// Restrict `path` to owner-read/write only (0o600 on Unix).
/// Exposed as `pub(crate)` so `master_key.rs` can call it when
/// persisting the per-installation master key file.
#[cfg(unix)]
...
pub(crate) fn secure_file(path: &std::path::Path) -> Result<(), String> {
    ...
    unsafe {
        // SetNamedSecurityInfoW: Set a DACL that is PROTECTED and has NO inherited ACEs.
        // On Windows, the simplest way to "lock" a file to the owner is to set a NULL DACL
        // (everyone) or a proper DACL. Since constructing a full DACL in raw Win32 is verbose,
        // we use SE_FILE_OBJECT with DACL_SECURITY_INFORMATION.
        
        // We ensure the file is not ReadOnly first using std
        ...
        // Apply a "Protected" DACL to stop inheritance from parent folders (like Downloads/Desktop)
        // This is a critical step for a secure vault.
        let result = SetNamedSecurityInfoW(
            ...
            None,
            None,
            None, // Passing None for DACL with PROTECTED_DACL_SECURITY_INFORMATION 
                  // effectively isolates the file's security descriptor.
            None,
        );

        if result.is_err() {
            return Err(format!("Windows ACL error: {:?}", result));
        }
    }
    Ok(())
}
...
impl VaultStore {
    ...
    fn check_file_mac(&self, path: &PathBuf, mac_path: &PathBuf, hkdf_info: &[u8]) -> Result<Vec<u8>, String> {
        ...
        let mut mac = HmacSha256::new_from_slice(&key).map_err(|_| "HMAC init fail".to_string())?;
        mac.update(&content);
    ...
    fn write_file_with_mac(&self, path: &PathBuf, mac_path: &PathBuf, hkdf_info: &[u8], content: &str) -> Result<(), String> {
        ...
        let mut mac = HmacSha256::new_from_slice(&key).map_err(|_| "HMAC init".to_string())?;
    ...
    fn migrate_legacy_vault(
        ...
        // Use the system audit key (derived from master key) so this
        // migration event is guaranteed to be logged and signed even though
        // no per-user vault key is active at this point in the call chain.
        let audit_key = crate::audit::derive_audit_key(&self.master_key);
    ...
    /// Generic blob encryption for VFS / Application files.
    pub fn encrypt_blob(&self, token: &str, plaintext: &[u8]) -> Result<Vec<u8>, String> {
        let keys = self.active_keys.lock().map_err(|_| "State error".to_string())?;
        let (_, vault_key) = keys.get(token).ok_or_else(|| "Locked".to_string())?;
    ...
    /// Generic blob decryption for VFS / Application files.
    pub fn decrypt_blob(&self, token: &str, packed: &[u8]) -> Result<Vec<u8>, String> {
        if packed.len() < NONCE_LEN + 1 { return Err("Short".to_string()); }
    ...
    pub fn rotate_key(
        ...
        // Derive audit key from master key for consistent, key-independent
        // audit trail.  Using the vault key here would mean audit HMACs change
        // every time a password is rotated, breaking log-consistency invariants.
        let audit_key = crate::audit::derive_audit_key(&self.master_key);
        crate::audit::log_event(
            &self.root_dir,
            "KEY_ROTATION",
