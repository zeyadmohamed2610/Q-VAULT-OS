...
def get_asset_path(filename: str) -> str:
    """Special helper to resolve the resources directory."""
    if not filename:
        return "" # Safe fallback for UI icons
    return get_resource_path(os.path.join("resources", filename))

def get_theme_data():
    """Returns the static theme definition from resources."""
    from resources import theme
    return theme

def get_wallpaper_registry():
    """Returns the static wallpaper metadata from resources."""
    from resources import wallpaper_data
