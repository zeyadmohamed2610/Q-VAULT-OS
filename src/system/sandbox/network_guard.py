...
class NetworkGuard:
    ...
    def request(
        ...
        """
        Safe HTTP request wrapper with Forensic Logging and Domain Lockdown.
        """
        self._enforce_rate_limit()
        if self.api:
        ...
        if attack_reason:
            self._log_forensic(f"NETWORK_ATTACK_BLOCKED: {method} {url} | Reason: {attack_reason}")
            return {"status": 403, "content": None, "error": f"Access Denied: {attack_reason}"}

        # 2. Permission & Domain Lockdown
        # Check if the app has a specific whitelist in its manifest
        is_allowed = False
        try:
            # Check PM_GUARD first for general 'network_access'
            PM_GUARD.check(self.app_id, "network_access", host)
            
            # Now check Domain Lockdown (Phase 15.2)
            # If the app has 'network_whitelist' in its record, enforce it strictly.
            record = RUNTIME_MANAGER.get_record(self.api.instance_id if self.api else self.app_id)
            if record and hasattr(record, "permissions"):
                whitelist = record.permissions.get("network_whitelist", [])
                if whitelist:
                    if any(allowed in host for allowed in whitelist):
                        is_allowed = True
                    else:
                        self._log_forensic(f"DOMAIN_LOCKDOWN_VIOLATION: {host} not in whitelist {whitelist}")
                        return {"status": 403, "content": None, "error": f"Domain Lockdown: {host} is not authorized."}
                else:
                    # No whitelist defined -> default allow if 'network_access' is granted
                    is_allowed = True
        except Exception as e:
            self._log_forensic(f"PERMISSION_DENIED: {host} | Error: {e}")
            return {"status": 403, "content": None, "error": str(e)}

        # 3. Forensic Telemetry
        self._log_forensic(f"OUTBOUND_REQUEST: {method} {host}")
        ...
        with token_ctx:
            try:
                ...
                self._log_forensic(f"REQUEST_FAILED: {host} | Error: {type(e).__name__}")
                return {"status": 0, "content": None, "error": str(e)}

    def _log_forensic(self, message: str):
        """Writes a signed entry to the Sovereign Audit Log."""
        try:
            from core.runtime_bridge import SECURITY_ENGINE
            if SECURITY_ENGINE:
                # Format: [APP_ID] Message
                log_msg = f"[{self.app_id}] {message}"
                SECURITY_ENGINE.log_audit("NET_GUARD", self.app_id, log_msg)
        except Exception:
            pass
