...
class Scheduler:
    ...
    def start(self) -> None:
        ...
        logger.info("[SCHEDULER] Started — listening for CLOCK_TICK & PROC_DEATH.")
    ...
    def _on_proc_death(self, payload: EventPayload) -> None:
        """Cleanup handler for process termination events."""
    ...
    def _on_tick(self, payload: EventPayload) -> None:
        """Main kernel logic entry point — called on every CLOCK_TICK."""
        try:
            ...
            import traceback
            err_msg = f"KERNEL PANIC: {str(e)}"
            logger.critical(f"[SCHEDULER] {err_msg}\n{traceback.format_exc()}")
            EVENT_BUS.emit(
                SystemEvent.INTERRUPT_RAISED, # Or a custom KERNEL_PANIC event if defined
                data={
                    "type": "PANIC",
                    "error": str(e),
    ...
    def _apply_aging(self) -> None:
        """
        Starvation prevention: Increment priority of processes that have
        been waiting in the ready queue for too long.
        """
        AGING_THRESHOLD = 20  # ticks
        with self._lock:
            for proc in self.ready_queue:
                # Calculate how long the process has been in the ready queue
                # since its last scheduling event or arrival.
                if proc.waiting_time > 0 and proc.waiting_time % AGING_THRESHOLD == 0:
                    if proc.priority < 10:  # Cap max priority
                        proc.priority += 1
                        logger.info(
                            f"[SCHEDULER] Aging: Boosted PID {proc.pid} priority to {proc.priority} "
    ...
    def schedule(self) -> None:
        ...
        if self.current_process is not None:
            ...
            # ── Step 2d: CPU Budget Watchdog (New) ──────────────
            cp.total_execution_ticks += 1
            if cp.cpu_budget > 0 and cp.total_execution_ticks > cp.cpu_budget:
                logger.warning(
                    f"[SCHEDULER][VIOLATION] PID {cp.pid} ({cp.name}) exceeded CPU budget ({cp.cpu_budget}u). "
                    f"Forcible termination engaged."
                )
                EVENT_BUS.emit(
                    SystemEvent.INTERRUPT_RAISED,
                    data={
                        "interrupt": {
                            "type": "CPU_HOG_VIOLATION",
                            "priority": 9,
                            "source_pid": cp.pid,
                            "tick": self._tick_count,
                            "details": f"Budget {cp.cpu_budget}u exceeded."
                        }
                    },
                    source="Scheduler"
                )
                # Terminate via Process Manager
