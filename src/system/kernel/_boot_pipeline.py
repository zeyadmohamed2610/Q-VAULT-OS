...
CORE_MODULES = [
    "ui.widgets.boot_screen",
    "ui.widgets.login_screen",
    "ui.widgets.desktop",
    "ui.widgets.lock_screen",
...
def _get_interrupt_manager():
    from system.kernel.interrupt_manager import INTERRUPT_MANAGER
...
def _get_dispatcher():
    from system.kernel.dispatcher import DISPATCHER
    return DISPATCHER
...
def _get_deadlock_manager():
    from system.kernel.deadlock_manager import DEADLOCK_MANAGER
...
KERNEL_MODULES = [
    "system.kernel.simulation_clock",
    "system.kernel.memory_manager",
    "system.kernel.interrupt_manager",
    "system.kernel.scheduler",
    "system.kernel.dispatcher",
    "system.kernel.multicore_engine",
    "system.kernel.deadlock_manager",
]
...
def boot_kernel():
    ...
    logger.info("  [KERNEL] Kernel Runtime Governance: ONLINE")
    logger.info(f"  [KERNEL] System Clock    — interval: {clock.tick_interval_ms}ms/tick")
    logger.info(f"  [KERNEL] Scheduler      — algorithm: {sched.algorithm}")
    logger.info(f"  [KERNEL] Memory         — {mm.total_size}u FIRST_FIT")
    logger.info(f"  [KERNEL] Multicore      — {mce.core_count} cores")
