...
class VirtualFilesystem:
    def __init__(self):
        ...
        self.vault_path = os.path.join(get_qvault_home(), "vault_data", "db", "vfs.bin")
        self._key = None
        self._init_tree()
    ...
    def _get_fernet(self):
        if not self._key:
            # In a real sovereign OS, this would be derived from user password
            # For now, we use a stable system key for persistence
            self._key = b'SovereignOS_StaticKey_For_VFS_Harden_2024=' 
        return Fernet(self._key)

    def save_to_vault(self):
        """Serialize and encrypt the entire VFS tree to disk."""
        def _serialize(node):
            return {
                "is_dir": node.is_dir,
                "content": node.content,
                "owner": node.owner,
                "children": {k: _serialize(v) for k, v in node.children.items()}
            }
        
        data = json.dumps(_serialize(self.root)).encode()
    ...
    def load_from_vault(self):
        ...
        try:
            ...
            def _deserialize(d):
                node = VNode(is_dir=d["is_dir"], content=d["content"], owner=d["owner"])
                for k, v in d["children"].items():
                    node.children[k] = _deserialize(v)
                return node
                
            self.root = _deserialize(data)
        except Exception:
            # If decryption fails (wrong key or corrupt), start fresh
            pass
