...
from system.errors import SecurityError
from system.runtime.pressure_manager import PressureManager
from system.runtime.resource_governor import ResourceGovernor
from system.runtime.quarantine import QuarantineManager
...
class AppRecord:
    ...
    def _get_current_cpu(self) -> float:
        """Calculates a realistic simulated CPU usage based on worker activity."""
        # Since apps share one process, we can't get OS-level per-app CPU.
        # We simulate it using worker load + base idling.
        workers = self.active_workers.get("total", 0)
        base = 0.5 # 0.5% base idling
        load = workers * 1.5 # 1.5% per active worker
        import random
        jitter = random.uniform(-0.2, 0.2)
        return round(max(base, load + jitter), 1)
...
class AppRuntimeManager:
    ...
    def __init__(self):
        ...
        # ── Sovereign Activity & Lockdown ──
        self.last_system_activity = time.time()
        self.is_system_locked = False
        self.lockdown_timeout = 300.0 # 5 minutes default
        
        # ── Sovereign Governance Core ──
    ...
    def _perform_cold_start_cleanup(self):
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                ...
                self.logger.debug(f"[Kernel] Failed to inspect/terminate process {proc.info.get('pid')}: {e}")
    ...
    def report_activity(self):
        """Update the last activity timestamp to prevent auto-lock."""
        self.last_system_activity = time.time()
        if self.is_system_locked:
            # We don't unlock automatically, but we stop the UI from being unresponsive
            pass
    ...
    def lock_system(self):
        ...
        with self._lock:
            if not self.is_system_locked:
                ...
                self.logger.warning("🚨 [SYSTEM_LOCK] System entered lockdown due to inactivity.")
                EVENT_BUS.emit(SystemEvent.SECURITY_ALERT, {"reason": "system_auto_lock"}, source="RuntimeManager")

    def unlock_system(self):
        """Recover from lockdown state."""
        with self._lock:
            self.is_system_locked = False
            self.last_system_activity = time.time()
            self.logger.info("🔓 [SYSTEM_UNLOCK] System unlocked by user.")

    def check_inactivity(self):
        """Monitor for timeout and trigger lock."""
        if self.is_system_locked: return
        if time.time() - self.last_system_activity > self.lockdown_timeout:
            self.lock_system()
    ...
    # ── Phase 13.7: Emergency & Duress ───────────────────────────────────────
    
    def trigger_duress_wipe(self):
        """
        CRITICAL: Performs a 'Silent Wipe' of the current session.
        Triggered by emergency code.
        """
        self.logger.critical("[DURESS] EMERGENCY PROTOCOL ACTIVATED. WIPING SESSION.")
        
        # 1. Immediate Lock
        self.lock_system()
        
        # 2. Kill all active applications
        for instance_id in list(self._registry.keys()):
            try:
                self.stop_app(instance_id)
            except Exception:
                pass
                
        # 3. Emit high-priority security alert
        EVENT_BUS.emit(SystemEvent.SECURITY_ALERT, {
            "type": "DURESS_WIPE",
            "severity": "CRITICAL",
            "message": "Session wiped via duress protocol."
        ...
        self.logger.info("[DURESS] System purged and secured.")
    ...
    def quarantine_app(self, instance_id: str, reason: str) -> None:
        """Isolate an application into a secure sandbox (Quarantine)."""
        record = self.get_record(instance_id)
        if not record: return
        
        with self._lock:
            record.state = AppState.QUARANTINED
            self.quarantine.quarantine_process(instance_id, reason)
            self.logger.critical(f"☣️ [QUARANTINE] {record.app_id} isolated. Reason: {reason}")
            
            self.notify(
                title=f"Quarantine: {record.app_id}",
                text=f"App isolated for security: {reason}",
                level="critical"
            )
            EVENT_BUS.emit(SystemEvent.SECURITY_ALERT, {
    ...
    def acquire_worker(self, instance_id: str, worker_type: str = "total", token: str = None) -> None:
        ...
        with self._lock:
            ...
            if self.is_system_locked:
                self.logger.warning(f"[LOCKDOWN] Blocked resource request from '{instance_id}'. System is LOCKED.")
                raise SecurityError("System is in Lockdown mode. Re-authentication required.")
    ...
    def spawn_process(self, instance_id: str, argv: list, **kwargs) -> Any:
        ...
        try:
            ...
            # Phase 16.9: Production Audit Trail
    ...
    def kill(self, instance_id: str) -> None:
        ...
        # Notify System
        self.notify("Process Terminated", f"The instance of {record.app_id} was closed.", level="warning")
    ...
    def get_explanation(self, instance_id: str) -> Dict[str, Any]:
        """Provides human-readable context for the trust score and status."""
        record = self.get_record(instance_id)
        if not record:
            return {"app_id": "Unknown", "trust_score": 0, "final_worker_limit": 0, "explanation": "No record found."}
        
        limit = self.get_worker_limit(instance_id)
        
        # Heuristic explanation
        exp = "App is behaving normally within sandbox parameters."
        if record.trust_score < 50:
            exp = "App has triggered multiple security penalties or resource spikes."
        if record.congested:
            exp = "App is hitting backpressure limits; processing is delayed."
        if record.state == AppState.QUARANTINED:
            exp = "CRITICAL: App isolated due to trust failure or suspected violation."
            
        return {
            "id": instance_id,
            "app_id": record.app_id,
            "trust_score": record.trust_score,
            "final_worker_limit": limit,
            "explanation": exp,
    ...
    def notify(self, title: str, text: str, level: str = "info") -> None:
        ...
        from core.event_bus import EVENT_BUS, SystemEvent
        evt_type = SystemEvent.EVT_INFO
        if level == "warning": evt_type = SystemEvent.EVT_WARNING
        elif level == "error": evt_type = SystemEvent.EVT_ERROR
        
        EVENT_BUS.emit(evt_type, {"title": title, "message": text}, source="RuntimeManager")
    ...
    def list_running(self) -> Dict[str, Any]:
        ...
        for instance_id, record in self._registry.items():
            ...
            apps_data.append({
                ...
                "cpu_usage": record._get_current_cpu(),
