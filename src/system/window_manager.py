...
class WindowManager(QObject):
    ...
    def focus_window(self, window_id: str):
        ...
        if hasattr(window, "is_minimized") and window.is_minimized:
            # BUG FIX: If we focus a minimized window, we MUST restore it properly
            # via the restoration pipeline so that UI/Animation reactions trigger.
    ...
    def minimize_window(self, window_id: str):
        ...
        if not window.is_minimized:
            ...
            # If we minimize the active window, clear active pointer and focus next
            if self._active == window_id:
                self._active = None
                self.cycle_windows() # Focus next available window
    ...
    def cycle_windows(self):
        ...
        # We want to find the next VISIBLE (non-minimized) window in the current workspace
        for wid in reversed(self._window_order):
            if wid in self._workspaces[self.current_workspace]:
                w = self._windows.get(wid)
                if w and not getattr(w, "is_minimized", False):
                    self.focus_window(wid)
                    return
        
        # If no visible windows remain, just clear active
        self._active = None
