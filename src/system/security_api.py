...
try:
    ...
    logger.warning(f"SecurityAPI: Rust core not available at {_bin_dir}. Falling back to Sovereign Simulation mode.")
    # We do NOT exit(1) anymore to ensure portability across devices during evaluation.
...
class SecurityAPI:
    ...
    def __init__(self):
        ...
        if _CORE_AVAILABLE:
            ...
            # Sovereign Simulation Mode with Dynamic User DB
            class MockEngine:
                def __init__(self):
                    self.users = {"admin": {"password": "admin123", "role": "admin", "display_name": "Administrator"}}
                
                def login(self, u, p): 
                    user = self.users.get(u)
                    if user and user["password"] == p:
                        return {"success": True, "value": f"sim-session-{u}"}
                    return {"success": False, "code": "AUTH_FAILED"}
                
                def logout(self, t): pass
                def get_system_status(self): return {"reasoning": "Nominal (Simulation)", "threat": 0}
                def create_user(self, t, u, p, r): 
                    self.users[u] = {"password": p, "role": r, "display_name": u.capitalize()}
                    return {"success": True}
                
                def update_user(self, u, new_password=None, new_name=None):
                    if u in self.users:
                        if new_password: self.users[u]["password"] = new_password
                        if new_name: self.users[u]["display_name"] = new_name
                        return {"success": True}
                    return {"success": False, "code": "USER_NOT_FOUND"}

                def list_users(self, t): 
                    return {"success": True, "value": [{"username": k, "role": v["role"]} for k,v in self.users.items()]}
            
            self._rust_engine = MockEngine()
    ...
    def logout(self) -> None:
        ...
        if self._token:
            try:
                ...
                logger.error(f"SecurityAPI: Logout from Rust engine failed: {e}")
            self._token = None
        ...
        try:
            ...
            logger.error(f"SecurityAPI: Failed to clear STATE: {e}")
        ...
        if hasattr(self, "_observers"):
            for cb in self._observers:
                try: cb(entry)
                    ...
                    logger.error(f"SecurityAPI: Observer callback failed: {e}")
...
def update_user(u: str, p: str = None, n: str = None): return get_security_api().update_user(u, p, n)
