...
def _panel(title: str, widget: QWidget, fixed_height: int = 0) -> QWidget:
    """Wrap widget in a titled deep glassmorphic panel card."""
    card = QFrame()
    card.setObjectName("kernel_panel")
    card.setStyleSheet(
        f"QFrame#kernel_panel {{ "
        f"background: rgba(13, 22, 36, 220); "
        f"border: 1px solid rgba(0, 230, 255, 0.1); "
    ...
    header = QHBoxLayout()
    title_lbl = _section_label(title)
    header.addWidget(title_lbl)
    header.addStretch()
    # Add a small neon dot for aesthetic
    dot = QLabel("●")
    dot.setStyleSheet(f"color: {THEME['primary_glow']}; font-size: 8px; background:transparent;")
...
class CPUTimelineWidget(QWidget):
    ...
    def closeEvent(self, event) -> None:
        ...
        EVENT_BUS.unsubscribe(SystemEvent.PROC_CONTEXT_SWITCHED, self._on_ctx_switch)
...
class MemoryMapWidget(QWidget):
    ...
    def _refresh_from_manager(self) -> None:
        try:
            from system.kernel.memory_manager import MEMORY_MANAGER as MM
    ...
    def closeEvent(self, event) -> None:
        EVENT_BUS.unsubscribe(SystemEvent.MEMORY_ALLOCATED, self._on_mem_change)
        EVENT_BUS.unsubscribe(SystemEvent.MEMORY_FREED,     self._on_mem_change)
        super().closeEvent(event)
...
class ReadyQueueWidget(QWidget):
    ...
    def closeEvent(self, event) -> None:
        EVENT_BUS.unsubscribe(SystemEvent.PROC_SCHEDULED,       self._on_sched_event)
        EVENT_BUS.unsubscribe(SystemEvent.PROC_PREEMPTED,       self._on_sched_event)
        EVENT_BUS.unsubscribe(SystemEvent.PROC_QUANTUM_EXPIRED, self._on_sched_event)
        super().closeEvent(event)
...
class DeadlockGraphWidget(QWidget):
    ...
    def refresh(self) -> None:
        try:
            from system.kernel.deadlock_manager import DEADLOCK_MANAGER as DM
    ...
    def closeEvent(self, event) -> None:
        EVENT_BUS.unsubscribe(SystemEvent.DEADLOCK_DETECTED, self._on_detected)
        EVENT_BUS.unsubscribe(SystemEvent.DEADLOCK_RESOLVED, self._on_resolved)
        super().closeEvent(event)
...
class CoreMonitorWidget(QWidget):
    ...
    def _refresh(self) -> None:
        try:
            from system.kernel.multicore_engine import MULTICORE_ENGINE as MCE
    ...
    def closeEvent(self, event) -> None:
        EVENT_BUS.unsubscribe(SystemEvent.CORE_ASSIGNED,    self._on_core_event)
        EVENT_BUS.unsubscribe(SystemEvent.PROCESS_MIGRATED, self._on_core_event)
        super().closeEvent(event)
...
class InterruptLogWidget(QWidget):
    ...
    def _on_irq(self, payload: EventPayload) -> None:
        try:
            ...
            color = C_ORANGE
            prefix = ""
            if itype == "CPU_HOG_VIOLATION":
                color = C_RED
                prefix = "⚠️ "
            
            self._add_row(
                f"{prefix}tick={tick:>4}  [{itype.upper():12s}]  prio={prio}  pid={pid}",
                color=color
            )
        except RuntimeError:
            pass
...
# ═══════════════════════════════════════════════════════════════
# 7. AUDIT VERIFY WIDGET
# ═══════════════════════════════════════════════════════════════

class AuditVerifyWidget(QWidget):
    """
    Forensic Tool: Verifies log integrity via Rust HMAC bridge.
    """
    def __init__(self, secure_api: SecureAPI = None, parent=None):
        ...
        self.input = QLabel("DROP LOG LINE HERE")
        self.input.setWordWrap(True)
        self.input.setStyleSheet(f"color: {C_DIM}; font-family: {MONO}; font-size: 9px; border: 1px dashed {C_BORDER_S}; padding: 8px;")
        
        self.status = QLabel("INTEGRITY: UNKNOWN")
        self.status.setStyleSheet(f"color: {C_MUTED}; font-family: {MONO}; font-size: 10px; font-weight: bold; padding: 4px;")
        
        btn = QPushButton("VERIFY LAST SYSTEM EVENT")
        btn.setStyleSheet(f"background: #1a2333; color: {C_CYAN}; border: 1px solid {C_CYAN}; font-family: {MONO}; font-size: 9px; padding: 4px;")
        btn.clicked.connect(self._verify_last)
        
        lay.addWidget(self.input)
        lay.addWidget(self.status)
        lay.addWidget(btn)

    def _verify_last(self):
        try:
            ...
            if os.exists(log_path):
                with open(log_path, "r", encoding="utf-8") as f:
                    ...
                    if lines:
                        ...
                        self.input.setText(last_line[:100] + "...")
                        if self.secure_api:
                            is_valid = self.secure_api.system.verify_audit_log(last_line)
                            if is_valid:
                                self.status.setText("INTEGRITY: ✅ VERIFIED")
                                self.status.setStyleSheet(f"color: {C_GREEN}; font-family: {MONO}; font-size: 10px; font-weight: bold;")
                            else:
                                self.status.setText("INTEGRITY: ❌ TAMPERED")
                                self.status.setStyleSheet(f"color: {C_RED}; font-family: {MONO}; font-size: 10px; font-weight: bold;")
        except Exception as e:
            self.status.setText(f"ERROR: {str(e)[:20]}")

# ═══════════════════════════════════════════════════════════════
# 8. CONSENT VIEWER WIDGET
# ═══════════════════════════════════════════════════════════════

class ConsentViewerWidget(QWidget):
    """
    Shows a list of temporary/active permissions granted by the user.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
...
# ═══════════════════════════════════════════════════════════════
# 9. RESOURCE PRESSURE WIDGET
# ═══════════════════════════════════════════════════════════════

class ResourcePressureWidget(QWidget):
    """
    Visualizes API call frequency and throttling for each app.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        self._apps: Dict[str, dict] = {} # instance_id -> {pressure, last_update}
        self.setMinimumHeight(100)
        
        # Decay timer
        self._decay = QTimer(self)
        self._decay.timeout.connect(self._do_decay)
        self._decay.start(500)

    def update_pressure(self, data: dict):
        iid = data.get("instance_id")
        calls = data.get("calls_per_sec", 0)
        delay = data.get("delay_ms", 0)
        
        # Pressure is 0-100 based on calls/sec (max 100)
        pressure = min(100, calls * 2)
        self._apps[iid] = {
    ...
    def _do_decay(self):
        ...
        changed = False
        to_del = []
        for iid, state in self._apps.items():
    ...
    def paintEvent(self, event):
        ...
        for iid, state in self._apps.items():
            ...
            # Label
            painter.setPen(QColor(C_DIM))
            painter.drawText(0, y, 100, 20, Qt.AlignVCenter, f"{app_id[:12]}")
            
            # Meter background
            meter_w = self.width() - 110
            painter.fillRect(100, y + 6, meter_w, 8, QColor(20, 30, 50))
            
            # Meter fill
            fill_w = int(meter_w * (p / 100))
            color = QColor(C_GREEN)
            if p > 80: color = QColor(C_RED)
            elif p > 50: color = QColor(C_ORANGE)
            
            painter.fillRect(100, y + 6, fill_w, 8, color)
            
            if delay > 0:
                painter.setPen(QColor(C_RED))
                painter.drawText(100 + meter_w - 40, y, 40, 20, Qt.AlignVCenter | Qt.AlignRight, f"{delay}ms")
...
class KernelMonitorApp(BaseApp, QWidget):
    ...
    consent_request_signal = pyqtSignal(dict)
    pressure_signal = pyqtSignal(dict)
    ...
    def __init__(self, secure_api: SecureAPI = None, parent=None):
        ...
        self.audit_verify   = AuditVerifyWidget(secure_api=secure_api)
        self.consent_viewer = ConsentViewerWidget()
        self.pressure_meter = ResourcePressureWidget()
    ...
    def _make_header(self) -> QWidget:
        ...
        self._integrity_lbl = QLabel("HEARTBEAT: —")
        self._integrity_lbl.setStyleSheet(
            f"color: {C_DIM}; font-family: {MONO}; font-size: 11px; font-weight: bold;"
        )
        ...
        row.addWidget(_vsep())
        row.addWidget(self._integrity_lbl)
        row.addWidget(_vsep())
        ...
        EVENT_BUS.subscribe(SystemEvent.REQ_USER_INPUT,    self._on_consent_request)
        EVENT_BUS.subscribe("ui.resource_pressure",        self._on_pressure)
    ...
    def _on_consent_request(self, payload: EventPayload) -> None:
        if payload.data.get("type") == "permission_consent":
            self.consent_request_signal.emit(payload.data)

    def _on_consent_request_safe(self, data: dict) -> None:
        ...
        msg.setWindowTitle("⬡ SOVEREIGN CONSENT REQUIRED")
        msg.setIcon(QMessageBox.Question)
        msg.setText(f"App '{app_id}' is requesting elevated privileges.")
        msg.setInformativeText(f"PERMISSION: <b>{perm}</b>\nJUSTIFICATION: {just}")
        msg.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
        msg.setDefaultButton(QMessageBox.No)
        
        # Style it to match theme
        msg.setStyleSheet(f"background: {C_SURFACE}; color: {C_TEXT}; font-family: {MONO};")
        
        res = msg.exec_()
        granted = (res == QMessageBox.Yes)
        
        EVENT_BUS.emit(
            "ui.permission_response",
            data={"request_id": req_id, "granted": granted},
            source="KernelMonitorApp"
        )
        self.consent_viewer.refresh()

    def _on_pressure(self, payload: EventPayload) -> None:
        self.pressure_signal.emit(payload.data)

    def _on_pressure_safe(self, data: dict) -> None:
        self.pressure_meter.update_pressure(data)
    ...
    def on_stop(self) -> None:
        ...
        # Comprehensive Unsubscription to prevent memory leaks
        unsubs = [
            (SystemEvent.CLOCK_TICK,        self._on_tick_header),
            (SystemEvent.DEADLOCK_DETECTED, self._on_deadlock),
            (SystemEvent.DEADLOCK_RESOLVED, self._on_resolved_header),
            (SystemEvent.REQ_USER_INPUT,    self._on_consent_request),
            ("ui.resource_pressure",        self._on_pressure),
        ]
    ...
    def _periodic_refresh(self) -> None:
        ...
        if self.secure_api:
            res = self.secure_api.system.verify_kernel_integrity()
            status = res.get("status", "ERROR")
            self._integrity_lbl.setText(f"HEARTBEAT: {status}")
            if status == "SECURE":
                self._integrity_lbl.setStyleSheet(f"color: {C_GREEN}; font-family: {MONO}; font-size: 11px; font-weight: bold;")
            else:
                self._integrity_lbl.setStyleSheet(f"color: {C_RED}; font-family: {MONO}; font-size: 11px; font-weight: bold;")
