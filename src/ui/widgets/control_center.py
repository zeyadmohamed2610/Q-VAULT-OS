...
from resources.theme import SPACE_MD, SPACE_SM, RADIUS_LG, MOTION_SNAPPY, MOTION_SMOOTH, EASE_OUT
...
class ControlCenter(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.btn_sleep = self._create_power_btn("icons/trust.svg", "Sleep", "#00e6ff")
        
        power_layout.addWidget(self.btn_shutdown)
        power_layout.addWidget(self.btn_restart)
        power_layout.addWidget(self.btn_sleep)
    ...
    def _create_power_btn(self, icon_path, label, color):
        ...
        """)
        if label == "Shutdown": 
            btn.clicked.connect(lambda: SystemControlHelper.power_action("shutdown"))
        elif label == "Restart":
            btn.clicked.connect(lambda: SystemControlHelper.power_action("restart"))
        elif label == "Sleep": 
            btn.clicked.connect(lambda: SystemControlHelper.power_action("sleep"))
        return btn
