...
class OSWindow(QWidget):
    ...
    def __init__(self, window_id: str, title: str, content_widget: QWidget, parent=None):
        super().__init__(parent)
        from resources.theme import SPACE_MD, RADIUS_MD, MOTION_SNAPPY, SPACE_XS
    ...
    def _setup_ui(self, window_id: str, title: str, content_widget: QWidget):
        ...
        self.title_bar.setFixedHeight(38) # Slightly taller for premium feel
        self.title_bar.setStyleSheet(f"""
            QWidget#TitleBar {{
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1, 
                    stop:0 rgba(25, 35, 50, 180), 
                    stop:1 rgba(15, 20, 30, 200));
        ...
        tb_layout.setContentsMargins(15, 0, 8, 0)
        ...
        # ───── CONTROLS ─────
        def create_ctrl(text, hover_color):
        ...
        self.btn_min = create_ctrl("−", "rgba(0, 240, 255, 0.2)")
    ...
    _RESIZE_M = 12   # Increased from 8 to make it easier to grab edges
    _MIN_W    = 400
    _MIN_H    = 300
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if direction and not is_flush:
                ...
                # FORCE: Grab mouse to ensure we keep receiving moves during resize
                self.grabMouse()
                event.accept()
                return
        
        # ── v1.0 Button Protection ──
        # If the user clicked a title bar button, let the button handle it.
        # This prevents focus events from firing during the click, which could
        # interrupt the minimize/close animation triggered by that same click.
        child = self.childAt(event.pos())
        if child and isinstance(child, QPushButton):
            # FIX: Still focus the window so the inactive overlay is removed
            # and the system knows which window is being manipulated.
    ...
    def mouseReleaseEvent(self, event):
        if self._resizing:
            ...
            self.releaseMouse() # Release grab
            if self.cursor().shape() != Qt.ArrowCursor:
                self.setCursor(Qt.ArrowCursor)
    ...
    def paintEvent(self, event):
        ...
        from resources.theme import RADIUS_MD
        from system.window_manager import WindowState
        ...
        if not self._is_active:
            ...
            painter.fillRect(rect, QColor(0, 0, 0, 120)) # Softer dim for inactive glass
            return
            
        # Active Glass Fill & Glow Border
        # Frosted glass background
        painter.setBrush(QBrush(QColor(10, 16, 28, 165))) # Frosted dark blue
        painter.setPen(QPen(QColor(THEME["primary_glow"] + "44"), 1.5))
        painter.drawRoundedRect(rect, radius, radius)
    ...
    def _final_close(self):
        ...
        except Exception as e: logger.debug(f"OSWindow: btn_max disconnect failed: {e}")
        try: self.title_bar.double_clicked.disconnect()
        except Exception as e: logger.debug(f"OSWindow: title_bar disconnect failed: {e}")
