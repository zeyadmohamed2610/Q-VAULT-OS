...
class MotionController:
    ...
    def fade_focus(window, active: bool):
        """
        v1.1 Focus Physics: Depth Scaling (0.99 -> 1.0) + Opacity (0.90 -> 1.0).
        Uses OutBack easing for an organic, premium feel.
        """
        if hasattr(window, "_focus_anim") and window._focus_anim.state() == QPropertyAnimation.Running:
            window._focus_anim.stop()
            
        target_opacity = 1.0 if active else 0.90
        geom = window.geometry()
        
        # Calculate subtle scale offset (0.99 for inactive)
        if active:
            # Restore to original calculated size (we assume current is the target if active)
            target_geom = geom
        else:
            # Shrink by 1% from center
            dw = int(geom.width() * 0.01)
            dh = int(geom.height() * 0.01)
            target_geom = QRect(
                geom.x() + dw//2,
                geom.y() + dh//2,
                geom.width() - dw,
        ...
        op_anim.setDuration(theme.MOTION_SNAPPY)
        op_anim.setStartValue(window.windowOpacity())
        op_anim.setEndValue(target_opacity)
        op_anim.setEasingCurve(theme.EASE_OUT)
        
        # Optional: Geometry scaling adds depth but can be jumpy if not handled by WM
        # For now, we stick to Opacity + subtle Border highlight via paintEvent.
        # However, to WOW the user, let's add a tiny scale pulse.
