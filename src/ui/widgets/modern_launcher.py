...
class ModernLauncher(QWidget):
    ...
    def _on_app_clicked(self, app_def):
        ...
        self.app_launched.emit(app_def)
        self.toggle() # Close with animation
    ...
    def toggle(self):
        if self.isVisible() and self.windowOpacity() > 0.5:
            ...
            g = self.geometry()
            self._scale_anim.setStartValue(g)
            self._scale_anim.setEndValue(QRect(g.x()+20, g.y()+20, g.width()-40, g.height()-40))
            ...
            if self.parent():
                p = self.parent()
                target_rect = QRect((p.width() - 600) // 2, (p.height() - 450) // 2, 600, 450)
                start_rect = QRect(target_rect.x() + 30, target_rect.y() + 30, 540, 390)
