...
from ui.apps.notepad.notepad_app import NotepadApp
...
def _best_mono_font(size: int = 12) -> QFont:
    """
...
class _Highlighter(QSyntaxHighlighter):
    def __init__(self, doc):
        ...
        self._add(r"/[a-zA-Z0-9_./-]+", THEME['primary_glow'], True) # Directories (Cyan)
        self._add(r"\b[a-zA-Z0-9_-]+\.[a-zA-Z0-9]+\b", THEME['text_dim']) # Files
        
        # 4. Flags & Arguments
        self._add(r" -[a-zA-Z]+", THEME['primary_soft']) # Flags
        self._add(r" --[a-zA-Z-]+", THEME['primary_soft']) # Long flags
        self._add(r"\".*\"", THEME['accent_purple']) # Strings
        self._add(r"'.*'", THEME['accent_purple'])
        
        # 5. ASCII Art Logo (Uniform Sovereign Glow - NO BOLD to keep alignment)
        self._add(r"[@!:.]{1,}", THEME['primary_glow'], False)
...
class TerminalBuffer(QPlainTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._suggestion = ""
        self.setCursorWidth(8) # Blocky cursor
    ...
    def append_output(self, text, newline=True):
        ...
        # ── Optimization ──────────────────────────────────────
        # Use a single cursor transaction and block signals to prevent 
        # UI stuttering on large outputs.
        cursor = self.textCursor()
        cursor.movePosition(QTextCursor.End)
        
        # Enhanced ANSI parser for multi-attribute codes (e.g., 1;32m)
        import re
        # Match \033[ followed by any number of digits/semicolons and then 'm'
        ...
        default_fmt.setForeground(QColor(THEME['text_main']))
        default_fmt.setFont(self.document().defaultFont())
        current_fmt = default_fmt
        
        parts = ansi_re.split(text)
        
        # Perform all insertions in one go
        cursor.beginEditBlock()
        try:
            # First part is always text before any ANSI code
            if parts[0]:
            ...
            for i in range(1, len(parts), 2):
                ...
                # Parse multiple codes (e.g., 1;36)
                codes = code_str.split(';')
                for code in codes:
                    if code == "0":
                        current_fmt = default_fmt
                    elif code == "1":
                        # Bold (we can't easily bold just a part with current current_fmt logic, 
                        # but we can try to merge)
                        current_fmt.setFontWeight(QFont.Bold)
                    elif code in colors:
                        current_fmt.setForeground(QColor(colors[code]))
    ...
    def contextMenuEvent(self, event):
        ...
        from resources.theme import THEME, FONT_MONO
        ...
        menu.setStyleSheet(f"""
            QMenu {{
                ...
                border-radius: 8px; padding: 4px; color: {THEME['text_main']};
                font-family: {FONT_MONO}; font-size: 10pt;
            }}
            QMenu::item {{ padding: 6px 20px; border-radius: 4px; margin: 1px 4px; }}
            QMenu::item:selected {{ background: {THEME['hover_glow']}; color: {THEME['primary_glow']}; }}
    ...
    def paintEvent(self, event):
        ...
        p.setPen(QColor(THEME['text_muted'])) # Muted grey
        p.setFont(self.font())
...
class TerminalApp(QWidget):
    ...
    def _update_state_ui(self, state):
        ...
        if state == EngineState.ROOT:
            ...
            self.state_label.setStyleSheet(f"color: {THEME['accent_error']};") 
            ...
            self.state_label.setStyleSheet(f"color: {THEME['text_disabled']};") 
        else:
