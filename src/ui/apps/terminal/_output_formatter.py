...
class OutputFormatter:
    ...
    @staticmethod
    def sudo_prompt() -> str:
        return "\033[1;36m[ ⊕ ]\033[0m Enter Governance Password for \033[1;32muser\033[0m: "
    ...
    def lock_screen() -> str:
        return (
            ...
            "\033[1;31m"
            "┌──────────────────────────────────────────┐\n"
            "│        Q-VAULT SESSION PROTECTION        │\n"
            "├──────────────────────────────────────────┤\n"
            "│  Status: [ LOCKED ]                      │\n"
            "│  Mode  : CRYPTO-HARDENED                 │\n"
            "└──────────────────────────────────────────┘\n"
            "\033[0m"
    ...
    def ls_output(entries: list[Path], base_path: Path | None = None) -> str:
        ...
        for x in sorted_entries:
            ...
            elif x.suffix in (".sh", ".py", ".bin", ".elf"):
                parts.append(f"[EXE] {name}*")
            elif x.suffix == ".exe":
                # Mask .exe as .bin or just hide extension
                name_clean = name[:-4]
                parts.append(f"[EXE] {name_clean}*")
            else:
                parts.append(f"      {name}")
    ...
    def subprocess_error(exc: Exception) -> str:
        msg = str(exc)
        # Prevent 'Terminal instance not found' or Windows paths in tracebacks
        if "Terminal" in msg or "wrapped object" in msg:
            return "\033[33m[Runtime] Signal synchronization delayed. Retrying sequence...\033[0m\n"
        from system.runtime.process_governor import PROCESS_GOVERNOR
        return f"sh: execution error: {PROCESS_GOVERNOR.sanitize_path(msg)}\n"
    ...
    def whoami(user: str, role: str) -> str:
        sym = "㉿" if role == "admin" else "λ"
        color = "\033[1;31m" if role == "admin" else "\033[1;32m"
        rst = "\033[0m"
        return f"[ {color}{sym}{rst} ] \033[1m{user}\033[0m (access: {role})\n"
    ...
    def prompt(user: str, role: str, cwd: Path, base_dir: Path) -> str:
        ...
        is_root = (role == "admin")
        sym     = "#" if is_root else "$"
        user_name = "root" if is_root else user
        ...
        user_col = "\033[31m" if is_root else "\033[32m"
        host_col = "\033[36m"
        path_col = "\033[34m"
        sym_col = "\033[31m" if is_root else "\033[32m"
        rst = "\033[0m"
        
        # Build prompt with Kali-style ㉿ separator and exact layout requested
        return f"┌──({user_col}{user_name}{rst}㉿{host_col}qvault{rst})──[{path_col}{path_str}{rst}]\n└─{sym_col}{sym}{rst} "