...
class VaultBrowser(BaseApp, QWidget):
    ...
    def _handle_external_http(self, url: str):
        ...
        if self.api:
            try:
                # Consult Rust-based AI Intel for domain reputation
                intel_res = self.api.intel.analyze_text(f"Audit URL reputation: {url}")
                # Naive mapping for demonstration; in prod, intel would return a 'score' field
                score = intel_res.get("threat_score", 0)
                if score > 70: trust_state = "risky"
                elif score > 20: trust_state = "unknown"
                else: trust_state = "trusted"
            except Exception:
                trust_state = "unknown"
        ...
        <div style='border-bottom: 2px solid {theme.PRIMARY}; margin-bottom: 20px; padding-bottom: 10px;'>
            ...
            <p style='color: {theme.TEXT_DIM}; font-size: 12px;'>Domain reputation: <b>{trust_state.upper()}</b>. JS execution neutralized.</p>
        </div>
        {safe_html}
        """
