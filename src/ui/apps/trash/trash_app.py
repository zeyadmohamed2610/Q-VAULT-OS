...
QPushButton {{
    background: rgba(255, 255, 255, 0.03); color: {THEME['text_dim']}; 
...
QPushButton:disabled {{ color: {THEME['text_disabled']}; border-color: transparent; }}
QPushButton#btnEmpty {{
    background: rgba(255, 51, 102, 0.1); border-color: {THEME['accent_error']}; color: {THEME['accent_error']};
}}
QPushButton#btnEmpty:hover {{ background: rgba(255, 51, 102, 0.25); }}
QListWidget {{
    background: {THEME['bg_black']}; color: {THEME['text_main']}; 
    border: 1px solid {THEME['border_subtle']};
    border-radius: 8px;
}}
QListWidget::item {{ padding: 8px 12px; border-radius: 4px; margin: 1px 4px; }}
QListWidget::item:selected {{ background: {THEME['hover_glow']}; color: {THEME['primary_glow']}; border: 1px solid {THEME['border_color']}; }}
...
class TrashApp(QWidget):
    ...
    def _setup_ui(self):
        ...
        title.setFont(QFont(FONT["family"], 14, QFont.Bold))
        header.addWidget(title)
        header.addStretch()
        self._count_badge = QLabel()
        self._count_badge.setStyleSheet(f"background:{THEME['bg_black']}; border-radius:10px; padding:2px 8px; font-size:11px;")
        ...
        self._empty_label.setStyleSheet(f"color: {THEME['text_muted']}; font-size: 16px;")
    ...
    def _context_menu(self, pos):
        ...
        if not items_sel:
            return
        menu = QMenu(self)
        menu.setStyleSheet(f"QMenu{{background:{THEME['bg_dark']};color:{THEME['text_main']};border:1px solid {THEME['border_color']};border-radius:6px;}}QMenu::item:selected{{background:{THEME['hover_glow']};color:{THEME['primary_glow']};}}")
